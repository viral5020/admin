...
const Splitscriptfilter = ({
    ...
    const handleAdd = async () => {
        if (!market) {
            toast.warning('Please select a market.', { autoClose: 3000 });
            return;
        }

        if (!isNseMarket && !script) {
            toast.warning('Please select a script.', { autoClose: 3000 });
            return;
        }

        if (!splitDate) {
            toast.warning('Please select a split date.', { autoClose: 3000 });
            return;
        }

        if (!splitNumber) {
            toast.warning('Please enter a split number.', { autoClose: 3000 });
            return;
        }
        ...
        try {
            ...
            if (response.data.success) {
                toast.success('Split data added successfully!', { autoClose: 3000 });
                handleClear(); // ✅ clear after success
            } else {
                toast.error(response.data.message || 'Failed to add split data.', { autoClose: 3000 });
            }
        } catch (error) {
            console.error('Error adding Split Data:', error);
            toast.error('An error occurred while adding split data.', { autoClose: 3000 });
        }
    };
