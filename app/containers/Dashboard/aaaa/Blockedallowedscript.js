...
import DailogBox from './helpers/DailogBox';
...
const Blockedallowedscript = () => {
    ...
    const handleAddOrder = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                market_type_id: typeof market === "object" ? market.id || market.value : market,
                script_id: typeof script === "object" ? script.id || script.value : script,
                user_id: client && typeof client === "object" ? client.id : client || "",
                master_user_id: master && typeof master === "object" ? master.id : master || "",
            };
    ...
    const handleConfirmRemoveSelected = async () => {
        console.log("handleConfirmRemoveSelected.......")
        // return;
    ...
    return (
        ...
        >
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        >
                                            <CardContent sx={{
                                                p: 1, "&:last-child": {
                                                    paddingBottom: 1, // remove bottom padding
                                                },
                                            }}>
            ...
            <Dialog open={openConfirm} onClose={handleCloseConfirm}>
                ...
                <DialogContent>
                    <DialogContentText>
                        Are you sure you want to delete {selectedLog ? `"${selectedLog.script_name}"` : ""}? <br />This action cannot be undone.
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
            ...
            {/* <Dialog open={openRemoveAllConfirm} onClose={handleCloseConfirm}>
                <DialogTitle>Confirm Deletion of All</DialogTitle>
                <DialogContent>
                    <DialogContentText>
                        Are you sure you want to delete
                        {addClient && (<> <br />Client : <i>{addClient.text}</i></>)}
                        {addMaster && (<> <br />Master : <i>{addMaster.text}</i></>)}
                        {addMarket && (<> <br />Market : <i>{addMarket.text}</i></>)}
                        {addScript.length && (<> <br />Script : <i>{addScript.text}</i></>)}
                        <br />This action cannot be undone.
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
                    <Button onClick={handleCloseConfirm}>Cancel</Button>
                    <Button onClick={handleConfirmRemoveSelected} color="error" variant="contained">Delete All</Button>
                </DialogActions>
            </Dialog> */}

            <DailogBox
                isOpen={openRemoveAllConfirm}
                handleClose={handleCloseConfirm}
                handleAction={handleConfirmRemoveSelected}
                title={`Confirm Deletion of All`}
                line_1={
                    <>
                        Are you sure you want to delete all scripts of <br />
                        {client && (<> <br /> Client : <i>{client.text}</i> </>)}
                        {master && (<> <br /> Master : <i>{master.text}</i> </>)}
                        {market && (<> <br /> Market : <i>{market.text}</i> </>)}
                        {script.length > 0 && (<> <br /> Script : <i>{script.text}</i> </>)}
                        <br />
                        <br />
                        <strong color='red'>* This action cannot be undone.</strong>
                    </>
                }
                actionBtnText='Delete All'
            ...
            <Dialog open={openRemoveAllDialog} onClose={handleCloseRemoveAllDialog} fullWidth maxWidth="sm" scroll="paper">
                ...
                <DialogActions sx={{ px: 3, pb: 2 }}>
                    ...
                    <Button onClick={handleRemoveAllConfirm} variant="contained" color="error">Remove</Button> {/*handleConfirmRemoveSelected*/}
                </DialogActions>
            </Dialog>
