import axios from "axios";
import { fetchClient } from "./fetchconfig";
import axiosInstance from "./axiosconfig";
import { constant, forex_market_type_id } from "../Watchlist/constant";
import { Bounce, ToastContainer, toast } from 'react-toastify';

export async function getDefaultParams() {
  const dataStored = JSON.parse(sessionStorage.getItem("data"));
  const { ip_address, user_agent } = await getUserInfo();
  const { isEmployeeLogin, isEmployeeLoginId, login_string, investor_status } = dataStored;
  const emp_para = { isEmployeeLogin, isEmployeeLoginId, login_string };

  return {
    is_app: "1",
    login_user_id: dataStored?.user_id,
    auth_key: dataStored?.auth_key,
    ip_address,
    user_agent,
    ...(isEmployeeLogin ? emp_para : {}),
    ...(investor_status ? { is_investor: true } : {}),
  }
}

export const changePasswordApi = async (api, current_password, new_password, confirm_password) => {
  const defaultParams = await getDefaultParams();
  try {
    const { data } = await axiosInstance.post(`/${api}`, { ...defaultParams, current_password, new_password, confirm_password });
    return data;
  } catch (err) {
    console.error("Error fetching in changePassword:", err);
  }
};

const getUserInfo = async () => {
  try {
    // Get IP Address
    const ipRes = await axios.get("https://api.ipify.org?format=json");
    const ip_address = ipRes.data.ip;
    const user_agent = navigator.userAgent;
    return { ip_address, user_agent };
  } catch (err) {
    console.error("Error fetching IP/UserAgent:", err);
    return { ip_address: null, user_agent: navigator.userAgent };
  }
};

export async function getIP() {
  try {
    const res = await axios.get("https://api64.ipify.org?format=json");
    console.log('res.data.ip', res.data.ip);
    return res.data.ip;
  } catch (error) {
    console.error("Failed to get IP address:", error);
  }
};

export const apifetchPositions = async (userId, authKey) => {
  const defaultParams = await getDefaultParams();
  try {
    const response = await axiosInstance.post("/datatables/position_book_list", {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: 0,
      iDisplayLength: 1000000,
      sSearch: "",
    });
    if (response.data && response.data.aaData) {
      return response.data.aaData;
    } else {
      return [];
    }
  } catch (error) {
    console.error("Error fetching position data:", error);
    return [];
  }
};

// ---------- PART 1 --------------

export const fetchLoginDataAPI = async (userId, authKey) => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post("/datatables/get_login_data_details", { ...defaultParams });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
    }

    if (response?.data?.status === "ok" && Array.isArray(response.data.data)) {
      return response.data.data.slice(0, 5);
    }
    return [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Login data fetch failed:", err);
    return [];
  }
};

export const fetchOrdersAPI = async (userId, authKey, type = "today", searchValue = "") => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: 0,
      iDisplayLength: 10000,
      sSearch: searchValue,
      isTodayTrade: type,
    };
    const { data } = await axiosInstance.post("/datatables/order_book_new", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data?.aaData || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching orders:", err);
    return [];
  }
};

// Fetch orders for a specific user (centralized wrapper for user profile screen)
export const fetchOrdersByUserAPI = async ({ target_user_id, pageSize = 50, start = 0 }) => {
  try {
    const defaultParams = await getDefaultParams();
    const { data } = await axiosInstance.post("/datatables/order_book_new", {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: start,
      iDisplayLength: pageSize,
      user_id: target_user_id || "",
      sSearch: "",
    });

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data?.aaData || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("fetchOrdersByUserAPI error:", err);
    return [];
  }
};

export const fetcholdOrdersAPI = async (userId, authKey, searchValue = "") => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      sEcho: 1,
      iDisplayStart: 0,
      iDisplayLength: 10000,
      sSearch: searchValue,
      ...defaultParams,
    };
    const { data } = await axiosInstance.post("/datatables/order_book_old", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data?.aaData || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching orders:", err);
    return [];
  }
};

export const fetcholdforexOrdersAPI = async (userId, authKey, searchValue = "") => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      sEcho: 1,
      iDisplayStart: 0,
      iDisplayLength: 10000,
      sSearch: searchValue,
      ...defaultParams,
    };
    const { data } = await axiosInstance.post("/datatables/order_book_forex_old", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data?.aaData || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching orders:", err);
    return [];
  }
};

export const fetchOrders1API = async ({
  userId,
  authKey,
  start_end = "",
  end_date = "",
  script_full_name = "",
  tradeType = ""
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      sEcho: 1,
      iDisplayStart: 0,
      iDisplayLength: 10,
      ...defaultParams,
      start_end,
      end_date,
      script_full_name,
      tradeType
    };
    const { data } = await axiosInstance.post("/datatables/order_book_new", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data?.aaData || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching orders:", err);
    return [];
  }
};

export const fetchforexOrdersAPI = async ({
  userId,
  authKey,
  filterType = "today",
  searchValue = "",
  currentPage = 0,
  pageSize = 10000,
  end_date = "",
  start_end = "",
  marketId = "",
  scriptIds = "",
  brokerId = "",
  masterUserId = "",
  clientId = "",
  status = "",
  orderType = ""
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchValue,
      ...defaultParams,
      isTodayTrade: filterType === "today" ? "today" : "",
      end_date,
      start_end,
      market_type_id: marketId || "",
      script_id: scriptIds,
      broker_id: brokerId || "",
      master_user_id: masterUserId || "",
      user_id: clientId || "",
      is_pending: status === "is_pending" ? "is_pending" : "",
      is_executed: status === "is_executed" ? "is_executed" : "",
      trade_type: orderType || "",
    };

    const { data } = await axiosInstance.post("/datatables/order_book_forex", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching forex orders:", err);
    return [];
  }
};

export const fetchPendingOrdersAPI = async (userId, authKey) => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post("/datatables/order_book_new", {
      is_pending: "true",
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: 0,
      iDisplayLength: 10,
      sSearch: "",
    });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    if (response?.data?.aaData) {
      return response.data.aaData;
    }
    return [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching pending orders:", err);
    return [];
  }
};

export const fetchRejectionLogsAPI = async (
  userId,
  authKey,
  filterType = "",
  searchQuery = "",
  pageSize,
  currentPage,
  marketId,
  scriptId,
  clientId,
  masterId,
  end_date,
  start_date,
) => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post("/datatables/rejection_log_view", {
      ...defaultParams,
      isTodayTrade: filterType,
      sSearch: searchQuery,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      market_type_id: marketId,
      script_id: scriptId,
      user_id: clientId,
      master_user_id: masterId,
      end_date: end_date,
      start_date: start_date,
    });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching rejection logs:", err);
    return [];
  }
};

export const fetchOrderlimitAPI = async (
  userId,
  authKey,
  filterType = "",
  searchQuery = "",
  pageSize,
  currentPage,
  marketId,
  scriptId,
  clientId,
  masterId,
  end_date,
  start_date,
) => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post("/datatables/client_order_limit_list", {
      sEcho: 1,
      ...defaultParams,
      isTodayTrade: filterType,
      sSearch: searchQuery,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
    });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching order limits:", err);
    return [];
  }
};

export const fetchBlockedAllowedAPI = async (
  userId,
  authKey,
  filterType = "",
  searchQuery = "",
  pageSize,
  currentPage,
  marketId,
  scriptId,
  clientId,
  masterId,
  end_date,
  start_date,
) => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post("/ajaxfiles/setting/list_client_block_script_list", {
      sEcho: 1,
      ...defaultParams,
      isTodayTrade: filterType,
      sSearch: searchQuery,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
    });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching blocked/allowed scripts:", err);
    return [];
  }
};

export const fetchTradesDataAPI = async (userId, authKey, scriptId) => {
  if (!scriptId) return [];
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      isTodayTrade: "today",
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: 0,
      iDisplayLength: 10,
      script_id: scriptId,
      user_id: selectedUserId,
      sSearch: "",
    };
    const { data } = await axiosInstance.post("/datatables/order_book_new", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data?.aaData || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch trades:", err);
    return [];
  }
};

export const fetchTrendStocksAPI = async (userId, authKey) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = { ...defaultParams };
    const { data } = await axiosInstance.post("/ajaxfiles/tranding_trades", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    if (data?.status === "ok" && data?.data) {
      return data.data.map((item) => ({
        Id: item.script_id,
        name: item.script_name || "N/A",
        ltp: item.ltp,
        per: item.per,
        rateChange: item.rateChange,
      }));
    }
    return [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch Scripts in Trends:", err);
    return [];
  }
};

export const fetchTradesAPI = async (userId, authKey, scriptId) => {
  if (!scriptId) return [];
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      isTodayTrade: "today",
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: 0,
      iDisplayLength: 10,
      script_id: scriptId,
      sSearch: "",
    };
    const { data } = await axiosInstance.post("/datatables/order_book_new", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data?.aaData || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch trades:", err);
    return [];
  }
};

export const fetchTopGainersLosersAPI = async (userId, authKey) => {
  if (!userId || !authKey) return { topGainers: [], topLosers: [] };
  try {
    const defaultParams = await getDefaultParams();
    const formData = { ...defaultParams };
    const { data } = await axiosInstance.post("/ajaxfiles/top_gainers_losers.php", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return { topGainers: [], topLosers: [] };
    }

    if (data?.status === "ok") {
      return {
        topGainers: data.topGainers || [],
        topLosers: data.topLosers || [],
      };
    }
    return { topGainers: [], topLosers: [] };
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ Failed to fetch top gainers/losers:", err);
    return { topGainers: [], topLosers: [] };
  }
};

export const fetchStockPositionsAPI = async (userId, authKey, scriptId) => {
  if (!scriptId) return [];
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      isActive: "active",
      sEcho: 1,
      iDisplayStart: 0,
      iDisplayLength: 10,
      script_id: scriptId,
      sSearch: "",
      ...defaultParams,
    };
    const { data } = await axiosInstance.post("/datatables/position_book_list", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data?.aaData || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching position ", err);
    return [];
  }
};

// Fetch positions for a specific user (centralized wrapper for user profile screen)
export const fetchPositionsByUserAPI = async ({ target_user_id, pageSize = 100000, start = 0 }) => {
  try {
    const defaultParams = await getDefaultParams();
    const { data } = await axiosInstance.post("/datatables/position_book_list", {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: start,
      iDisplayLength: pageSize,
      user_id: target_user_id || "",
      sSearch: "",
    });

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data?.aaData || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("fetchPositionsByUserAPI error:", err);
    return [];
  }
};

export const fetchDashboardDataAPI = async (userId, authKey) => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post("/ajaxfiles/dashboard_count_trade", { ...defaultParams });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return null;
    }

    if (response?.data?.status === "ok") {
      return {
        today_rejection: response.data.today_rejection,
        total_rejection: response.data.total_rejection,
        total_position: response.data.total_position,
        today_trades: response.data.today_trades,
        week_trades: response.data.week_trades,
        today_pending_trades: response.data.today_pending_trades,
      };
    }
    return null;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch dashboard ", err);
    return null;
  }
};

export const setScriptBlockSettingAPI = async (userId, authKey, market_type_id, script_ids) => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post('/ajaxfiles/setting/set_script_block_setting', {
      ...defaultParams,
      market_type_id,
      script_ids,
    });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('Error setting script block:', err);
    throw err;
  }
};

export const removeBlockListAPI = async (userId, authKey, script_block_id) => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post('/ajaxfiles/setting/set_script_block_setting', {
      ...defaultParams,
      script_block_id,
    });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('Error removing block list:', err);
    throw err;
  }
};

export const fetchStrikeDataAPI = async ({ expiry, script, index = 0, term = 'CE' }) => {
  if (!term || !expiry?.expiry_date || !script?.script_id) {
    throw new Error('Missing term, expiry or script data');
  }
  try {
    const defaultParams = await getDefaultParams();
    const expiry_id = `${expiry.script_expiry_id}-${index}`;
    const response = await axiosInstance.post('/ajaxfiles/get_option_strike_price', {
      ...defaultParams,
      expiry_id,
      term,
    });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data?.data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('Error fetching strike ', err);
    return [];
  }
};

export const closeAllPositions = async ({
  password,
  market,
  script,
  client,
  master,
  broker,
  exparyDate,
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      password,
      market_type_id: market,
      script_id: script,
      user_id: client,
      master_user_id: master,
      broker_id: broker,
      expiry_date: exparyDate,
      search_val: '',
      allow_close_all: true,
      ...defaultParams
    };
    const res = await axiosInstance.post('/ajaxfiles/trade_exit_all_position1', payload);

    if (res?.data?.status === "error") {
      toast.error(res?.data?.message || "Something went wrong");
      throw new Error(res.data.message || "Operation failed");
    }

    if (res.data.success) {
      return res.data;
    } else {
      throw new Error(res.data.message || 'Failed to close positions');
    }
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('Error in closeAllPositions:', err.message || err);
    throw err;
  }
};

export const forexcloseAllPositions = async ({
  password,
  market,
  script,
  client,
  master,
  broker,
  exparyDate,
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      password,
      market_type_id: market,
      script_id: script,
      user_id: client,
      master_user_id: master,
      broker_id: broker,
      expiry_date: exparyDate,
      search_val: '',
      allow_close_all: true,
      ...defaultParams
    };
    const res = await axiosInstance.post('/ajaxfiles/trade_exit_all_position1_forex', payload);

    if (res?.data?.status === "error") {
      toast.error(res?.data?.message || "Something went wrong");
      throw new Error(res.data.message || "Operation failed");
    }

    if (res.data.success) {
      return res.data;
    } else {
      throw new Error(res.data.message || 'Failed to close positions');
    }
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('Error in forexcloseAllPositions:', err.message || err);
    throw err;
  }
};

// ---------- PART 2 --------------

export const rolloverPositions = async ({
  password,
  market,
  script,
  client,
  master,
  broker,
  exparyDate,
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      password,
      market_type_id: market,
      script_id: script,
      user_id: client,
      master_user_id: master,
      broker_id: broker,
      expiry_date: exparyDate,
      ...defaultParams,
    };
    const res = await axiosInstance.post('/ajaxfiles/trade_roll_over_all', payload);

    if (res?.data?.status === "error") {
      toast.error(res?.data?.message || "Something went wrong");
      throw new Error(res.data.message || "Rollover failed");
    }

    if (res.data.success) {
      return res.data;
    } else {
      throw new Error(res.data.message || "Failed to roll over positions");
    }
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('Error in rolloverPositions:', err.message || err);
    throw err;
  }
};

function isChanged(apiData) {
  const data = sessionStorage.getItem('notification');
  if (JSON.stringify(apiData) === data) {
    return;
  } else {
    // Optional auto-reload logic can be added here later
  }
}

export const checkLoginAPI = async (isJustLogin, user_id, auth_key) => {
  try {
    let defaultParams = await getDefaultParams();
    if (isJustLogin) {
      defaultParams = { ...defaultParams, login_user_id: user_id, auth_key };
    }
    if (!(defaultParams?.auth_key || defaultParams?.login_user_id)) {
      return;
    }

    const response = await axiosInstance.post("/ajaxfiles/check_login", { ...defaultParams });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error in checkLogin:", err);
    throw err;
  }
};

export const fetchNotificationAPI = async (isJustLogin, user_id, auth_key) => {
  try {
    let defaultParams = await getDefaultParams();
    if (isJustLogin) {
      defaultParams = { ...defaultParams, login_user_id: user_id, auth_key };
    }
    if (!(defaultParams?.auth_key || defaultParams?.login_user_id)) return;

    const response = await axiosInstance.post("/ajaxfiles/setting/fetch_notification", { ...defaultParams });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return response.data;
    }

    // Optional: isChanged(response.data);
    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error in fetchNotificationAPI:", err);
    throw err;
  }
};

// stored in sessionstorage > first_password_changed === 0 > navigate to changePassword
export const getWatchListDataAPI = async () => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post('/ajaxfiles/market_watch_list1', { ...defaultParams });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('Error in getting watchlist data:', err);
    throw err;
  }
};

export async function getMarketWatchFilterAPI() {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post('/ajaxfiles/get_market_watch_filter2', { ...defaultParams });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('getMarketWatchFilterAPI error:', err);
    throw err;
  }
}

export async function getMarketWiseScriptForexAPI(marketId) {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post('/ajaxfiles/get_market_wise_script_forex', {
      ...defaultParams,
      market_type_id: marketId,
    });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('getMarketWiseScriptForexAPI error:', err);
    throw err;
  }
}

export async function getScriptWiseExpiryForexAPI(marketId, scriptId) {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post('/ajaxfiles/get_script_wise_expiry_forex', {
      ...defaultParams,
      market_type_id: marketId,
      script_id: scriptId,
    });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('getScriptWiseExpiryForexAPI error:', err);
    throw err;
  }
}

export const getForexWatchListDataAPI = async () => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post('/ajaxfiles/market_watch_list_forex', { ...defaultParams });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('Error in getting forex watchlist data:', err);
    throw err;
  }
};

export const addPosition = async (payload) => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post('/ajaxfiles/setting/set_user_type_qty_single', {
      ...defaultParams,
      ...payload,
    });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('Error in Add Position data:', err);
    throw err;
  }
};

export const updateTrade = async ({ trade_id, trade_rate, trade_lot, trade_qty, device_type = 0 }) => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post('/ajaxfiles/trade_edit', {
      trade_id,
      trade_rate,
      trade_lot,
      trade_qty,
      device_type,
      ...defaultParams,
    });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('Error in updating trade:', err);
    throw err;
  }
};

export const deleteTrade = async ({ trade_id, password = '', device_type = 0 }) => {
  try {
    const rawData = sessionStorage.getItem("data");
    if (!rawData) throw new Error("User data not found.");
    let parsedData;
    try {
      parsedData = JSON.parse(rawData);
    } catch (e) {
      throw new Error("Failed to parse user data.");
    }
    const { user_id: userId, auth_key: authKey } = parsedData;
    if (!userId || !authKey) throw new Error("Credentials missing.");

    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post('/ajaxfiles/trade_delete', {
      ...defaultParams,
      trade_id,
      password,
      device_type,
    });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('Error deleting trade:', err);
    throw err;
  }
};

export const fetchPositionsAPI = async ({
  login_user_id,
  auth_key,
  all_outstanding,
  expiry_date,
  group_by,
  market_type_id,
  script_id,
  broker_id,
  master_user_id,
  user_id
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: 0,
      iDisplayLength: 100000,
      sSearch: "",
      all_outstanding,
      expiry_date,
      group_by,
      market_type_id,
      script_id,
      broker_id,
      master_user_id,
      user_id
    };

    const response = await axiosInstance.post("/datatables/position_book_list_forex", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error in fetchPositionsAPI:", err);
    return null;
  }
};

export const fetchforexTradesDataAPI = async (userId, authKey, scriptId) => {
  if (!scriptId) return [];
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      isTodayTrade: "today",
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: 0,
      iDisplayLength: 10,
      script_id: scriptId,
      sSearch: "",
    };
    const { data } = await axiosInstance.post("/datatables/order_book_forex", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data?.aaData || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch forex trades:", err);
    return [];
  }
};

export const fetchSummaryReportAPI = async (
  user_id, master_user_id, broker_id, end_date, start_end, market_type_id, script_id, valan_id
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      broker_id,
      master_user_id,
      user_id,
      end_date,
      start_end,
      market_type_id,
      script_id,
      valan_id,
    };
    const { data } = await axiosInstance.post("/ajaxfiles/summary_report", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data?.data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch summary report:", err);
    return [];
  }
};

export const fetchSelfplAPI = async (
  user_id, master_user_id, broker_id, end_date, start_end, market_type_id, script_id, valan_id
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      broker_id,
      master_user_id,
      user_id,
      end_date,
      start_end,
      market_type_id,
      script_id,
      valan_id,
    };
    const { data } = await axiosInstance.post("/ajaxfiles/self_profit_and_loss_report", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data?.data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch self P&L report:", err);
    return [];
  }
};

export const fetchforexSummaryReportAPI = async (
  user_id, master_user_id, broker_id, end_date, start_end, market_type_id, script_id, valan_id
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      broker_id,
      master_user_id,
      user_id,
      end_date,
      start_end,
      market_type_id,
      script_id,
      valan_id,
    };
    const { data } = await axiosInstance.post("/ajaxfiles/summary_report_forex", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data?.data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch forex summary report:", err);
    return [];
  }
};

export const fetchMarginManagementListAPI = async (userId, authKey, client, master, broker) => {
  if (!userId || !authKey) return [];
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      broker_id: broker?.id,
      master_user_id: master?.id,
      user_id: client?.id,
      ...defaultParams,
    };
    const { data } = await axiosInstance.post("/ajaxfiles/margin_management_list", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data?.data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch margin management list:", err);
    return [];
  }
};

export const fetchforexMarginManagementListAPI = async (userId, authKey, client, master, broker) => {
  if (!userId || !authKey) return [];
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      broker_id: broker?.id,
      master_user_id: master?.id,
      user_id: client?.id,
      ...defaultParams,
    };
    const { data } = await axiosInstance.post("/ajaxfiles/margin_management_forex_list", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data?.data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch forex margin management list:", err);
    return [];
  }
};

export const tradePlaceAPI = async (dataObj) => {
  try {
    const dataStored = JSON.parse(sessionStorage.getItem("data"));
    const defaultParams = await getDefaultParams();

    const payload = {
      ...defaultParams,
      device_type: 0,
      market_type_id: dataObj.market_type_id,
      script_id: dataObj.script_id,
      script_expiry_id: dataObj.script_expiry_id,
      trade_rate: dataObj.price,
      trade_qty: dataObj.qty,
      trade_lot: dataObj.lot,
      trade_type: dataObj.market,
      trade_type_x: dataObj.tradeType,
      check_script_name: dataObj.script_expiry_type
        ? `${dataObj.script_name}-${dataObj.script_expiry_type}`
        : dataObj.script_name,
      user_id: (dataStored?.user_type !== 1 && dataStored?.user_type !== 2)
        ? dataObj.client?.id
        : undefined,
    };

    const response = await axiosInstance.post('/ajaxfiles/trade_place', payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('Error in tradePlaceAPI:', err);
    throw err;
  }
};

// ---------- PART 3 --------------

export const addMarketScriptAPI = async ({
  market_type_id,
  script_id,
  script_expiry_id,
  expiryTerm,
  type,
  strickObj,
}) => {
  try {
    const defaultParams = await getDefaultParams();
    let payload;

    if (String(market_type_id) === constant) {
      payload = {
        market_type_id,
        script_id,
        script_expiry_id: `${script_expiry_id}-${expiryTerm}`,
        script_expiry_type: strickObj.check_script_name,
        script_expiry_orginal_formate: `${strickObj.rate} ${type}`
      };
    } else if (market_type_id == forex_market_type_id) {
      payload = {
        market_type_id,
        script_id,
      };
    } else {
      const str = 'I'.repeat(expiryTerm + 1);
      payload = {
        market_type_id,
        script_id,
        script_expiry_id,
        script_expiry_type: str,
      };
    }

    const res = market_type_id == forex_market_type_id
      ? await axiosInstance.post('/ajaxfiles/add_market_watch_forex', { ...defaultParams, ...payload })
      : await axiosInstance.post('/ajaxfiles/add_market_watch', { ...defaultParams, ...payload });

    if (res?.data?.status === "error") {
      toast.error(res?.data?.message || "Something went wrong");
      throw new Error(res.data.message || "Failed to add market script");
    }

    if (res.data.status === 'ok') {
      return res.data;
    } else {
      throw new Error(res.data.message || "Unknown error occurred");
    }
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('Error in addMarketScript:', err.message || err);
    throw err;
  }
};

export async function removeMarketWatchAPI(market_watch_id) {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post('/ajaxfiles/remove_market_watch', { ...defaultParams, market_watch_id });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.log('err', err);
    throw err;
  }
}

export async function favouriteActionAPI(market_watch_id, action_type) {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post('/ajaxfiles/favourite_upde', { ...defaultParams, market_watch_id, action_type });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.log('err', err);
    throw err;
  }
}

export const fetchValanNamesApi = async (term) => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post("/ajaxfiles/get_valan_name_search", { ...defaultParams, term });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data.results || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching Valan IDs:", err);
    return [];
  }
};

export const fetchUserlistingAPI = async (
  currentPage, rowsPerPage, tradeAfter, tradeBefore, loginBefore, loginAfter, broker, master, user, status, searchText
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * rowsPerPage,
      iDisplayLength: rowsPerPage,
      sSearch: searchText || "",
      loginBefore,
      loginAfter,
      tradeBefore,
      tradeAfter,
      broker,
      master,
      user,
      status,
    };
    const { data } = await axiosInstance.post("/datatables/user_list_key", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch user list:", err);
    return [];
  }
};

export const fetchEmployeelistingAPI = async (
  currentPage, rowsPerPage, tradeAfter, tradeBefore, broker, master, user, status, searchText
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * rowsPerPage,
      iDisplayLength: rowsPerPage,
      sSearch: searchText || "",
      tradeBefore,
      tradeAfter,
      broker,
      master,
      user,
      status,
    };
    const { data } = await axiosInstance.post("/datatables/employee_list_key", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch employee list:", err);
    return [];
  }
};

export const fetchMasterlistingAPI = async (
  currentPage, rowsPerPage, joinAfter, joinBefore, loginBefore, loginAfter, broker, master, status, searchText
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * rowsPerPage,
      iDisplayLength: rowsPerPage,
      sSearch: searchText || "",
      loginBefore,
      loginAfter,
      joinBefore,
      joinAfter,
      broker,
      master,
      status,
    };
    const { data } = await axiosInstance.post("/datatables/master_list_key", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch master list:", err);
    return [];
  }
};

export const fetchBrokerlistingAPI = async (
  currentPage, rowsPerPage, joinAfter, joinBefore, loginBefore, loginAfter, broker, master, status, searchText
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * rowsPerPage,
      iDisplayLength: rowsPerPage,
      sSearch: searchText || "",
      loginBefore,
      loginAfter,
      joinAfter,
      joinBefore,
      broker,
      master,
      status,
    };
    const { data } = await axiosInstance.post("/datatables/broker_list2", formData);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }

    return data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch broker list:", err);
    return [];
  }
};

// Utility function for API call
export const tradeEditDeleteLogLogsAPI = async (
  currentPage, pageSize, searchText, market, scriptIds, master, client, end_date, start_date, is_deleted, is_updated, is_admin
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
      market_type_id: market?.id,
      script_id: scriptIds,
      master_user_id: master?.id,
      user_id: client?.id,
      end_date,
      start_date,
      is_deleted,
      is_updated,
      is_admin,
    };
    const response = await axiosInstance.post("/datatables/trade_log_view.php", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch logs:", err);
    throw err;
  }
};

export const tradeAutosquareofAPI = async (
  currentPage, pageSize, searchText, market, scriptIds, master, client, end_date, start_date, is_deleted, is_updated, is_admin
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
      market_type_id: market?.id,
      script_id: scriptIds,
      master_user_id: master?.id,
      user_id: client?.id,
      end_date,
      start_date,
      is_deleted,
      is_updated,
      is_admin,
    };
    const response = await axiosInstance.post("/datatables/auto_closed_report", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch auto-square-off report:", err);
    throw err;
  }
};

export const MtmalertsAPI = async (
  currentPage, pageSize, searchText, market, scriptIds, master, client, end_date, start_date, is_deleted, is_updated, is_admin
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
      market_type_id: market?.id,
      script_id: scriptIds,
      master_user_id: master?.id,
      user_id: client?.id,
      end_date,
      start_date,
      is_deleted,
      is_updated,
      is_admin,
    };
    const response = await axiosInstance.post("/datatables/mtm_alert_top20", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch MTM alerts:", err);
    throw err;
  }
};

export const ipaddresslogAPI = async (
  currentPage, pageSize, searchText, market, scriptIds, master, client, end_date, start_date, is_deleted, is_updated, is_admin
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
      market_type_id: market?.id,
      script_id: scriptIds,
      master_user_id: master?.id,
      user_id: client?.id,
      end_date,
      start_date,
      is_deleted,
      is_updated,
      is_admin,
    };
    const response = await axiosInstance.post("/datatables/same_ip_list", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch IP address log:", err);
    throw err;
  }
};

export const cashledgerAPI = async (
  currentPage, pageSize, searchText, market, scriptIds, master, client, end_date, start_date, is_deleted, is_updated, is_admin
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
      market_type_id: market?.id,
      script_id: scriptIds,
      master_user_id: master?.id,
      user_id: client?.id,
      end_date,
      start_date,
      is_deleted,
      is_updated,
      is_admin,
    };
    const response = await axiosInstance.post("/datatables/cash_ledger_list", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch cash ledger:", err);
    throw err;
  }
};

export const cashEntryAPI = async (
  currentPage, pageSize = 10, searchText = '', user_type = '', user_id = '', start_date = '', end_date = '', cash_add = ''
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
      user_type,
      user_id,
      start_date,
      end_date,
      cash_add,
    };
    const response = await axiosInstance.post("/datatables/cash_ledger_list", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch cash entries:", err);
    throw err;
  }
};

export const JVEntryAPI = async (
  currentPage, pageSize, searchText, market, scriptIds, master, client, date, date_to, is_deleted, is_updated, is_admin, cash_add
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
      market_type_id: market?.id,
      script_id: scriptIds,
      master_user_id: master?.id,
      user_id: client?.id,
      date,
      date_to,
      is_deleted,
      is_updated,
      is_admin,
      cash_add,
    };
    const response = await axiosInstance.post("/ajaxfiles/fetch_jv_entries", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch JV entries:", err);
    throw err;
  }
};

export const BillfilterAPI = async ({
  currentPage,
  pageSize = 10,
  term = "",
  valan_id = valanId?.id,
  amount = "",
  start_date = "",
  end_date = "",
  market_type_id = "",
  user_id = "",
  broker_user_id = "",
  master_user_id = "",
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      term,
      master_user_id,
      broker_user_id,
      valan_id,
      market_type_id,
      user_id,
      start_date,
      end_date,
      amount,
    };
    const response = await axiosInstance.post("/ajaxfiles/bill_filter_amount_wise", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching bill filter data:", err);
    return [];
  }
};

export const CrosstradelogAPI = async ({
  currentPage,
  pageSize = 10,
  valan_id,
  start_date = "",
  end_date = "",
  market_type_id = "",
  script_id,
  user_id = "",
  broker_user_id = "",
  master_user_id = "",
  term = "",
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      term,
      valan_id,
      market_type_id,
      script_id,
      user_id,
      master_user_id,
      broker_user_id,
      start_date,
      end_date,
    };
    const response = await axiosInstance.post("/ajaxfiles/cross_trade_data", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching cross trade log:", err);
    return [];
  }
};

export const tradeEditLoglistAPI = async (
  currentPage, pageSize, searchText, master, client, end_date, start_date, is_admin
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
      master_user_id: master?.id,
      user_id: client?.id,
      end_date,
      start_date,
      is_admin,
    };
    const response = await axiosInstance.post("/datatables/user_edit_log_list", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch trade edit logs:", err);
    throw err;
  }
};

export const editDeleteLogLogsAPI = async (
  currentPage, pageSize, searchText, market, scriptIds, master, client, end_date, start_date, is_deleted, is_updated, is_admin
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
      market_type_id: market?.id,
      script_id: scriptIds,
      master_user_id: master?.id,
      user_id: client?.id,
      end_date,
      start_date,
      is_deleted,
      is_updated,
      is_admin,
    };
    const response = await axiosInstance.post("/datatables/trade_log_view.php", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch edit/delete logs:", err);
    throw err;
  }
};

export const ValanLogsAPI = async (
  currentPage, pageSize, searchText, market, scriptIds, master, client, end_date, start_date, is_deleted, is_updated
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
    };
    const response = await axiosInstance.post("/ajaxfiles/setting/list_valan_master", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch Valan logs:", err);
    throw err;
  }
};

export const editDeleteoldLogsAPI = async (
  currentPage, pageSize, searchText, market, scriptIds, master, client, end_date, start_date, is_deleted, is_updated
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
      market_type_id: market?.id,
      script_id: scriptIds,
      master_user_id: master?.id,
      user_id: client?.id,
      end_date,
      start_date,
      is_deleted,
      is_updated,
    };
    const response = await axiosInstance.post("/datatables/trade_log_view_old", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch old edit/delete logs:", err);
    throw err;
  }
};

// ---------- PART 4 --------------

export const AdminloginlistAPI = async (
  currentPage,
  pageSize,
  searchText,
  market,
  scriptIds,
  master,
  client,
  end_date,
  start_date,
  is_deleted,
  is_updated,
  is_admin,
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
      market_type_id: market?.id,
      script_id: scriptIds,
      master_user_id: master?.id,
      user_id: client?.id,
      end_date,
      start_date,
      is_deleted,
      is_updated,
      is_admin,
    };
    const response = await axiosInstance.post("/ajaxfiles/setting/admin_login_list", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch admin login list:", err);
    throw err;
  }
};

export const CasheditDeleteLogsAPI = async (
  currentPage,
  pageSize,
  searchText,
  market,
  scriptIds,
  master,
  client,
  end_date,
  start_date,
  is_deleted,
  is_updated,
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
      market_type_id: market?.id,
      script_id: scriptIds,
      master_user_id: master?.id,
      user_id: client?.id,
      end_date,
      start_date,
      is_deleted,
      is_updated,
    };
    const response = await axiosInstance.post("/datatables/cash_ledger_log_list", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch cash edit/delete logs:", err);
    throw err;
  }
};

export const manualtradesAPI = async (
  currentPage,
  pageSize,
  searchText,
  market,
  scriptIds,
  master,
  client,
  end_date,
  start_date,
  is_deleted,
  is_updated,
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
      market_type_id: market?.id,
      script_id: scriptIds,
      master_user_id: master?.id,
      user_id: client?.id,
      end_date,
      start_date,
      is_deleted,
      is_updated,
    };
    const response = await axiosInstance.post("/datatables/order_book_manual", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch manual trades:", err);
    throw err;
  }
};

export const bulktradingAPI = async ({
  currentPage,
  pageSize = 10,
  sSearch = '',
  master_user_id = '',
  broker_user_id = '',
  market_type_id = '',
  script_id = '',
  user_id = '',
  start_date = '',
  end_date = '',
  noOfTrades = '',
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch,
      master_user_id,
      broker_user_id,
      market_type_id,
      script_id,
      user_id,
      start_date,
      end_date,
      noOfTrades,
    };

    const response = await axiosInstance.post("/ajaxfiles/bulk_trading_report", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return {
        status: 'error',
        data: [],
        minimum: 0,
        message: response.data.message || 'Something went wrong',
      };
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch bulk trading report:", err);
    return {
      status: 'error',
      data: [],
      minimum: 0,
      message: err.message || 'Something went wrong',
    };
  }
};

export const addAccountAPI = async (payload) => {
  try {
    const defaultParams = await getDefaultParams();
    const { data } = await axiosInstance.post("/ajaxfiles/create_user", { ...defaultParams, ...payload });

    if (data?.status === "error") {
      toast.error(data?.message || "Failed to create account");
      return data; // Return full error object for form handling
    }

    return data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error creating account:", err);
    return {
      status: 'error',
      message: err?.response?.data?.message || "Network error occurred",
    };
  }
};

export const getMarketScriptForAddAccountAPI = async () => {
  try {
    const defaultParams = await getDefaultParams();
    const { data } = await axiosInstance.post("/ajaxfiles/get_mcx_script_type", { ...defaultParams });

    if (data?.status === "error") {
      toast.error(data?.message || "Failed to fetch market scripts");
      return [];
    }

    return data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching market scripts:", err);
    return [];
  }
};

console.log("API.js runn.......");

const apiCache = new Map();

export const fetchOptionsAPI = async (url, params) => {
  const key = `${url}:${JSON.stringify(params)}`;
  if (apiCache.has(key)) {
    console.log("Returning cached response for:", key);
    return apiCache.get(key);
  }

  try {
    const { data } = await axiosInstance.post(url, params);
    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return [];
    }
    const result = data.results || [];
    apiCache.set(key, result);
    return result;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error(`Error fetching from ${url}`, err);
    return [];
  }
};

export const fetchOptionsPriceOptionAPI = async (url, params) => {
  const key = `${url}:${JSON.stringify(params)}`;
  if (apiCache.has(key)) {
    console.log("Returning cached response for:", key);
    return apiCache.get(key);
  }

  try {
    const { data } = await axiosInstance.post(url, params);
    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return null;
    }
    apiCache.set(key, data);
    return data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error(`Error fetching from ${url}`, err);
    return null;
  }
};

export const fetchOptionsAPIManualScript = async (url, params) => {
  const key = `${url}:${JSON.stringify(params)}`;
  if (apiCache.has(key)) {
    console.log("Returning cached response for:", key);
    return apiCache.get(key);
  }

  try {
    const { data } = await axiosInstance.post(url, params);
    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return null;
    }
    console.log("datadata=", data);
    apiCache.set(key, data);
    return data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error(`Error fetching from ${url}`, err);
    return null;
  }
};

export const editAccountAPI = async (payload) => {
  try {
    const defaultParams = await getDefaultParams();
    const { data } = await axiosInstance.post("/ajaxfiles/edit_user", { ...defaultParams, ...payload });

    if (data?.status === "error") {
      toast.error(data?.message || "Failed to update account");
      return data;
    }

    return data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error updating account:", err);
    return {
      status: 'error',
      message: err?.response?.data?.message || "Network error occurred",
    };
  }
};

export const getUserDetailsAPI = async (user_id) => {
  try {
    const defaultParams = await getDefaultParams();
    const { data } = await axiosInstance.post("/ajaxfiles/view_user_details", { ...defaultParams, user_id });

    if (data?.status === "error") {
      toast.error(data?.message || "Failed to fetch user details");
      return null;
    }

    return data || null;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching user details:", err);
    return null;
  }
};

export const getemployeeDetailsAPI = async (user_id) => {
  try {
    const defaultParams = await getDefaultParams();
    const { data } = await axiosInstance.post("/ajaxfiles/view_user_details_emp", { ...defaultParams, user_id });

    if (data?.status === "error") {
      toast.error(data?.message || "Failed to fetch employee details");
      return null;
    }

    return data || null;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching employee details:", err);
    return null;
  }
};

export const fetchPermissionsAPI = async () => {
  try {
    const defaultParams = await getDefaultParams();
    const res = await axiosInstance.post("/ajaxfiles/get_emp_permission", defaultParams);

    if (res?.data?.status === "error") {
      toast.error(res?.data?.message || "Something went wrong");
      return [];
    }

    return Array.isArray(res.data.emp_per) ? res.data.emp_per : [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error fetching permissions:", err);
    return [];
  }
};

export const ScriptwiselotAPI = async (
  currentPage,
  pageSize,
  searchText,
  market,
  scriptIds,
  master,
  client,
  end_date,
  start_date,
  is_deleted,
  is_updated,
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
    };
    const response = await axiosInstance.post("/ajaxfiles/setting/list_script_master", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch script-wise lot settings:", err);
    throw err;
  }
};

export const StopfuturelistAPI = async (
  currentPage,
  pageSize,
  searchText,
  market,
  scriptIds,
  master,
  client,
  end_date,
  start_date,
  is_deleted,
  is_updated,
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
    };
    const response = await axiosInstance.post("/datatables/list_future_block", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch stop future list:", err);
    throw err;
  }
};

export const ExpiryvalidationAPI = async (
  currentPage,
  pageSize,
  searchText,
  market,
  scriptIds,
  master,
  client,
  end_date,
  start_date,
  is_deleted,
  is_updated,
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
    };
    const response = await axiosInstance.post("/ajaxfiles/setting/list_expiry_validation", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch expiry validation list:", err);
    throw err;
  }
};

export const TimesettingAPI = async (
  currentPage,
  pageSize,
  searchText,
  market,
  scriptIds,
  master,
  client,
  end_date,
  start_date,
  is_deleted,
  is_updated,
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
    };
    const response = await axiosInstance.post("/ajaxfiles/setting/list_start_end_time", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch time settings:", err);
    throw err;
  }
};

export const NSEOPTmanageAPI = async (
  currentPage,
  pageSize,
  searchText,
  market,
  scriptIds,
  master,
  client,
  end_date,
  start_date,
  is_deleted,
  is_updated
) => {
  try {
    const defaultParams = await getDefaultParams();
    const formData = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
    };
    const response = await axiosInstance.post("/datatables/option_block_list", formData);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Something went wrong");
      return [];
    }

    return response.data?.aaData || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Failed to fetch NSE Option manage list:", err);
    return [];
  }
};

// api.js
export const fetchPositionDataAPI = async ({
  user_id,
  auth_key,
  selectedMarket,
  selectedScripts,
  searchText,
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      market: selectedMarket?.name || "",
      scripts: selectedScripts.map((s) => s.name).join(","),
      sSearch: searchText.trim(),
    };

    const { data } = await axiosInstance.post("/ajaxfiles/get_script_wise_qty1", payload);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return { data: [], status: "error" };
    }

    return data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call failed", err);
    throw err;
  }
};

export const fetchBlockedScriptsAPI = async ({
  user_id,
  auth_key,
  selectedMarket,
  script,
  searchText,
  formatScriptIds,
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      market: selectedMarket?.name || "",
      script_id: formatScriptIds(script),
      search_text: searchText.trim(),
    };

    const { data } = await axiosInstance.post("/ajaxfiles/setting/list_block_script.php", payload);

    if (data?.status === "error") {
      toast.error(data?.message || "Something went wrong");
      return { data: [], status: "error" };
    }

    return data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call failed", err);
    throw err;
  }
};

// ---------- PART 5 --------------

export const confirmTradeAPI = async ({
  user_id,
  auth_key,
  broker,
  master,
  client,
  valanId,
  password,
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      broker_id: broker?.id || "",
      master_id: master?.id || "",
      user_id: client?.id || "",
      valan_id: valanId?.id || "",
      password,
    };
    console.log("🔍 Confirm Trade Payload:", payload);
    const { data } = await axiosInstance.post("/ajaxfiles/brokerage_refresh", payload);

    if (data?.status === "error") {
      toast.error(data?.message || "Failed to confirm trade");
      return data;
    }

    return data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Confirm Trade) failed", err);
    throw err;
  }
};

export const fetchLedgerDetailsAPI = async ({ user_id, auth_key, targetUserId }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      user_id: targetUserId,
    };
    console.log("🔍 Fetching Ledger Details with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/get_user_valan_wise_bill", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to fetch ledger details");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Ledger Details) failed", err);
    throw err;
  }
};

// ===== Broker Listing auxiliary actions =====
export const resetPasswordAPI = async ({ user_id }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      user_id,
    };
    const response = await axiosInstance.post("/ajaxfiles/reset_password", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to reset password");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (reset_password) failed", err);
    throw err;
  }
};

export const changeUserStatusAPI = async ({ user_id }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      user_id,
    };
    const response = await axiosInstance.post("/ajaxfiles/change_user_status", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to change user status");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (change_user_status) failed", err);
    throw err;
  }
};

export const clearLoginAttemptsAPI = async ({ user_id }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      user_id,
    };
    const response = await axiosInstance.post("/ajaxfiles/clear_login_attempts", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to clear login attempts");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (clear_login_attempts) failed", err);
    throw err;
  }
};

// Employee-specific admin actions
export const resetEmployeePasswordAPI = async ({ user_id }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = { ...defaultParams, user_id };
    const { data } = await axiosInstance.post('/ajaxfiles/reset_password_emp', payload);

    if (data?.status === "error") {
      toast.error(data?.message || "Failed to reset employee password");
      return data;
    }

    return data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('❌ API call (reset_password_emp) failed', err);
    throw err;
  }
};

export const changeEmployeeStatusAPI = async ({ user_id }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = { ...defaultParams, user_id };
    const { data } = await axiosInstance.post('/ajaxfiles/change_emp_status', payload);

    if (data?.status === "error") {
      toast.error(data?.message || "Failed to change employee status");
      return data;
    }

    return data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error('❌ API call (change_emp_status) failed', err);
    throw err;
  }
};

export const setInvestorPasswordAPI = async ({ user_id, current_password, password }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      user_id,
      current_password,
      password,
    };
    const response = await axiosInstance.post("/ajaxfiles/investor_password_set", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to set investor password");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (investor_password_set) failed", err);
    throw err;
  }
};

export const removeInvestorPasswordAPI = async ({ user_id, current_password }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      user_id,
      current_password,
    };
    const response = await axiosInstance.post("/ajaxfiles/investor_password_remove", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to remove investor password");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (investor_password_remove) failed", err);
    throw err;
  }
};

export const fetchJVAPI = async ({ user_id, auth_key, start_date, end_date }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      date: start_date || "21-08-2025",
      date_to: end_date || "28-08-2025",
    };
    console.log("🔍 Fetching Logs with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/fetch_jv_entries", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to fetch JV entries");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Fetch Logs) failed", err);
    throw err;
  }
};

export const fetchLedgerAPI = async ({ user_id, auth_key, selectedUserId, filters }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...filters,
      ...defaultParams,
      user_id: selectedUserId,
    };
    console.log("🔍 Fetching Ledger with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/get_valan_wise_ledger.php", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to fetch ledger");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Fetch Ledger) failed", err);
    throw err;
  }
};

export const fetchSportsLedgerAPI = async ({ user_id, auth_key }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: 0,
      iDisplayLength: 100,
      sSearch: "",
    };
    console.log("🔍 Fetching Sports Ledger with payload:", payload);
    const response = await axiosInstance.post("/datatables/cricket_account_statement", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to fetch sports ledger");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Sports Ledger) failed", err);
    throw err;
  }
};

export const updateCnbcLinkAPI = async ({ user_id, auth_key, link }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      link,
      ...defaultParams,
    };
    console.log("🔍 Updating CNBC link with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/setting/update_cnbc", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to update CNBC link");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Update CNBC Link) failed", err);
    throw err;
  }
};

export const fetchLedgerReportAPI = async ({ user_id, auth_key, filters }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...filters,
      ...defaultParams,
    };
    console.log("🔍 Fetching Ledger Report with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/ledger_report", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to fetch ledger report");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Ledger Report) failed", err);
    throw err;
  }
};

export const uploadUserTypeQtyAPI = async ({ user_id, auth_key, file }) => {
  try {
    const formData = new FormData();
    formData.append("file", file);
    formData.append("login_user_id", user_id);
    formData.append("auth_key", auth_key);

    console.log("🔍 Uploading file:", file.name);
    const response = await axiosInstance.post(
      "/ajaxfiles/setting/upload_user_type_qty",
      formData,
      { headers: { "Content-Type": "multipart/form-data" } }
    );

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to upload file");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Upload User Type Qty) failed", err);
    throw err;
  }
};

export const confirmTradeManualAPI = async ({
  user_id,
  auth_key,
  password,
  trade_date,
  addMarket,
  addScript,
  lot,
  quantity,
  price,
  pair,
  addClient,
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const scriptArray = Array.isArray(addScript) ? addScript : [addScript];
    const payload = {
      ...defaultParams,
      password: password || "",
      trade_date,
      high_low: 0,
      market_type_id: addMarket?.id || "",
      device_type: 0,
      script_id: scriptArray[0]?.value || "",
      script_expiry_id: scriptArray[0]?.script_expiry_id || "",
      trade_lot: lot || 1,
      trade_qty: quantity || 0,
      trade_rate: price || 0,
      trade_type: pair === "buy" ? 1 : 0,
      with_broker: 1,
      check_script_name: scriptArray.map((s) => s.script_name).join(","),
      user_id: addClient?.id || user_id || "",
    };
    console.log("🔍 Submitting manual trade with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/trade_manual", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to place manual trade");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Manual Trade) failed", err);
    throw err;
  }
};

export const updateMarqueeMessageAPI = async ({ user_id, auth_key, message }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      message,
      ...defaultParams,
    };
    console.log("🔍 Updating Marquee Message with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/setting/marquee_add", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to update marquee message");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Update Marquee Message) failed", err);
    throw err;
  }
};

export const uploadUserTypeQtyMasterAPI = async (file) => {
  try {
    const formData = new FormData();
    formData.append("file", file);
    console.log("🔍 Uploading CSV file:", file.name);
    const { data } = await axiosInstance.post(
      "/ajaxfiles/setting/upload_user_type_qty_master",
      formData,
      { headers: { "Content-Type": "multipart/form-data" } }
    );

    if (data?.status === "error") {
      toast.error(data?.message || "Failed to upload master CSV");
      return data;
    }

    return data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Upload CSV) failed", err);
    throw err;
  }
};

export const fetchScriptQtyListAPI = async ({
  user_id,
  auth_key,
  currentPage = 0,
  pageSize = 10,
  searchText = "",
  selectedUserLevel,
  marketName,
  scriptName,
  positionLimit,
  minOrder,
  maxOrder,
  minBet,
  maxBet,
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      sEcho: 1,
      iDisplayStart: currentPage * pageSize,
      iDisplayLength: pageSize,
      sSearch: searchText,
      user_level: selectedUserLevel,
      market_name: marketName,
      script_name: scriptName,
      position_limit: positionLimit,
      min_order: minOrder,
      max_order: maxOrder,
      min_bet: minBet,
      max_bet: maxBet,
    };
    console.log("🔍 Fetching Script Qty List with payload:", payload);
    const response = await axiosInstance.post("/datatables/script_qty_list", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to fetch script qty list");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Script Qty List) failed", err);
    return [];
  }
};

// ---------- PART 6 --------------
export const fetchUserLevelsAPI = async ({ user_id, auth_key }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = { ...defaultParams };
    console.log("🔍 Fetching user levels with payload:", payload);
    const res = await axiosInstance.post("/ajaxfiles/get_user_level", payload);

    if (res?.data?.status === "error") {
      toast.error(res?.data?.message || "Failed to fetch user levels");
      return [];
    }

    return Array.isArray(res.data)
      ? res.data
      : Array.isArray(res.data?.data)
        ? res.data.data
        : [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (User Levels) failed", err);
    return [];
  }
};

// 🔹 Fetch filters (markets + scripts)
export const fetchMarketWatchFiltersAPI = async ({ user_id, auth_key }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = { ...defaultParams };
    console.log("🔍 Fetching Market Watch Filters with payload:", payload);
    const res = await axiosInstance.post("/ajaxfiles/get_market_watch_filter", payload);

    if (res?.data?.status === "error") {
      toast.error(res?.data?.message || "Failed to fetch market watch filters");
      return {};
    }

    return res.data || {};
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Market Watch Filters) failed", err);
    return {};
  }
};

export const addNotificationAPI = async ({ user_id, auth_key, user_type, title, message }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      user_type,
      title,
      message,
    };
    console.log("🔍 Adding notification with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/setting/add_notification", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to add notification");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Add Notification) failed", err);
    throw err;
  }
};

export const removeBlockOptionExpiryAPI = async ({ script_expiry_option_id }) => {
  try {
    const payload = {
      script_expiry_option_id,
      is_block: 0,
    };
    console.log("🔍 Removing block option with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/setting/add_block_option_expiry", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to unblock expiry option");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Remove Block Option) failed", err);
    throw err;
  }
};

// Generic block/unblock option expiry API
export const setBlockOptionExpiryAPI = async ({ script_expiry_option_id, is_block }) => {
  try {
    const payload = { script_expiry_option_id, is_block };
    console.log("🔍 Setting block option with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/setting/add_block_option_expiry", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to update block status");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Set Block Option) failed", err);
    throw err;
  }
};

export const addClientOrderLimitAPI = async ({
  user_id,
  auth_key,
  market_type_id,
  script_id,
  client_user_id,
  master_user_id,
  price_percent,
  value,
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      market_type_id,
      script_id,
      user_id: client_user_id,
      master_user_id,
      price_percent,
      value,
    };
    console.log("🔹 Sending payload to add_client_order_limit:", payload);
    const response = await axiosInstance.post("/ajaxfiles/setting/add_client_order_limit", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to add order limit");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Add Client Order Limit) failed", err);
    throw err;
  }
};

// 🔹 Delete client order limit
export const deleteClientOrderLimitAPI = async ({ user_id, auth_key, client_order_id }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      client_order_id,
    };
    console.log("🔹 Sending payload to delete_client_order_limit:", payload);
    const response = await axiosInstance.post("/ajaxfiles/setting/delete_client_order_limit", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to delete order limit");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Delete Client Order Limit) failed", err);
    throw err;
  }
};

export const deleteFutureTradingBlockAPI = async ({ future_id, user_id, auth_key }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      future_id,
      ...defaultParams,
    };
    console.log("🔍 Deleting future trading block with payload:", payload);
    const response = await axiosInstance.post("/setting/remove_future_trading_block", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to delete future trading block");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Delete Future Trading Block) failed", err);
    throw err;
  }
};

export const fetchBulkTradeListAPI = async ({ user_id, auth_key, noOfTrades }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      noOfTrades,
    };
    console.log("🔍 Fetching bulk trade list with payload:", payload);
    const response = await axiosInstance.post("/datatables/bulk_trade_list", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to fetch bulk trade list");
      return [];
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Bulk Trade List) failed", err);
    return [];
  }
};

// 🔹 Save/update bulk trading settings
export const saveBulkTradingSettingsAPI = async ({ user_id, auth_key, noOfTrades }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      no_of_trade: noOfTrades,
    };
    console.log("🔍 Saving bulk trading settings with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/setting/set_bulk_trading", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to save bulk trading settings");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Save Bulk Trading Settings) failed", err);
    throw err;
  }
};

export const addClientBlockScriptAPI = async ({
  user_id,
  auth_key,
  market_type_id,
  script_id,
  client_user_id,
  master_user_id,
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      market_type_id,
      script_id,
      user_id: client_user_id,
      master_user_id,
    };
    console.log("🔹 Adding client block script with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/setting/set_client_block_script_setting", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to block script for client");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Add Client Block Script) failed", err);
    throw err;
  }
};

// 🔹 Delete client block script setting
export const deleteClientBlockScriptAPI = async ({ user_id, auth_key, client_block_script_id }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      client_block_script_id,
    };
    console.log("🔹 Deleting client block script with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/setting/remove_client_block_script_setting", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to delete client block script");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Delete Client Block Script) failed", err);
    throw err;
  }
};

// 🔹 Remove selected client block script setting
export const removeSelectedClientBlockScriptAPI = async ({
  user_id,
  auth_key,
  market_type_id,
  script_id,
  client_user_id,
  master_user_id,
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      market_type_id,
      script_id,
      user_id: client_user_id,
      master_user_id,
    };
    console.log("🔹 Removing selected client block script with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/setting/remove_client_block1_script_setting", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to remove selected block");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Remove Selected Client Block Script) failed", err);
    throw err;
  }
};

export const addReceiptAPI = async ({
  user_id,
  auth_key,
  user_type,
  entry_user_id,
  type,
  date1,
  amount,
  remarks = "",
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      user_type,
      user_id: entry_user_id,
      type,
      date1,
      amount,
      remarks,
    };
    console.log("🔹 Adding receipt with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/add_receipt", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to add receipt");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Add Receipt) failed", err);
    throw err;
  }
};

// 🔹 Edit entry/receipt
export const editReceiptAPI = async ({
  user_id,
  auth_key,
  entry_id,
  entry_user_id,
  type,
  date1,
  amount,
  remarks = "",
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      entry_id,
      user_id: entry_user_id,
      type,
      date1,
      amount,
      remarks,
    };
    console.log("🔹 Editing receipt with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/edit_receipt", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to edit receipt");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Edit Receipt) failed", err);
    throw err;
  }
};

// 🔹 Delete entry/receipt
export const deleteReceiptAPI = async ({ user_id, auth_key, entry_id, entry_user_id }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      entry_id,
      user_id: entry_user_id,
    };
    console.log("🔹 Deleting receipt with payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/delete_receipt", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to delete receipt");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Delete Receipt) failed", err);
    throw err;
  }
};

export const addJVEntryAPI = async ({
  login_user_id,
  auth_key,
  from_ledger,
  to_ledger,
  ledger_type,
  date1,
  amount,
  remarks = "",
  jv_entry_id = 0,
  jv_entry_time = ""
}) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      from_ledger,
      to_ledger,
      ledger_type,
      date1,
      amount,
      remarks,
      jv_entry_id,
      jv_entry_time
    };
    console.log("🔹 Add JV payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/add_jv", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to add JV entry");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Add JV) failed", err);
    throw err;
  }
};

// 🔹 Update JV entry (same endpoint as add, but with jv_entry_id)
export const updateJVEntryAPI = async (payload) => addJVEntryAPI(payload);

// 🔹 Delete JV entry
export const deleteJVEntryAPI = async ({ login_user_id, auth_key, entryId }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      entryId
    };
    console.log("🔹 Delete JV payload:", payload);
    const response = await axiosInstance.post("/ajaxfiles/delete_jv_entry", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to delete JV entry");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (Delete JV) failed", err);
    throw err;
  }
};

const fetchUserLog = async ({ endpoint, user_id, log_datetime }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      user_id,
      log_datetime
    };
    console.log(`🔹 Fetching from ${endpoint} with payload:`, payload);
    const response = await axiosInstance.post(endpoint, payload); // already relative

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to fetch log data");
      return [];
    }

    return response.data.data || [];
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error(`❌ API call failed for ${endpoint}`, err);
    return [];
  }
};

// Fetch basic edit log
export const fetchBasicLogAPI = async ({ login_user_id, auth_key, user_id, log_datetime }) =>
  fetchUserLog({
    endpoint: "/ajaxfiles/setting/user_basic_edit_log",
    user_id,
    log_datetime
  });

// Fetch brokerage edit log
export const fetchBrokerageLogAPI = async ({ login_user_id, auth_key, user_id, log_datetime }) =>
  fetchUserLog({
    endpoint: "/ajaxfiles/setting/user_commission_edit_log",
    user_id,
    log_datetime
  });

// Fetch market edit log
export const fetchMarketLogAPI = async ({ login_user_id, auth_key, user_id, log_datetime }) =>
  fetchUserLog({
    endpoint: "/ajaxfiles/setting/user_market_edit_log",
    user_id,
    log_datetime
  });

export const fetchSummaryApi = async (tab = "Stock", selectedUser = {}) => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post("/view_market_type_data", {
      ...defaultParams,
      view_user_id: selectedUser?.id || "",
      tabs: tab
    });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to fetch summary");
      return null;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("API fetchSummaryApi error:", err);
    throw err;
  }
};

export const fetchSummaryAPI = async ({ login_user_id, auth_key, view_user_id, tab }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      view_user_id,
      tabs: tab
    };
    console.log("🔹 Fetching summary:", payload);
    const res = await axiosInstance.post("/ajaxfiles/view_market_type_data", payload);

    if (res?.data?.status === "error") {
      toast.error(res?.data?.message || "Failed to fetch summary");
      return null;
    }

    return res.data?.status === "ok" ? res.data.data : null;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ Summary fetch error:", err);
    throw err;
  }
};

export const fetchProfileAPI = async (para) => {
  try {
    const defaultParams = await getDefaultParams();
    const view_user_id = para?.view_user_id;
    const payload = {
      ...defaultParams,
      ...(view_user_id ? { view_user_id } : {})
    };

    const res = await axiosInstance.post("/ajaxfiles/view_user_profile", payload);

    if (res?.data?.status === "error") {
      toast.error(res?.data?.message || "Failed to fetch profile");
      return { profile: null, loginIps: [] };
    }

    if (res.data.status === "ok") {
      return {
        profile: res.data.data,
        loginIps: res.data.loginIps || []
      };
    }
    return { profile: null, loginIps: [] };
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ Profile fetch error:", err);
    throw err;
  }
};

export const removeMappedAccount = async ({ login_user_id, auth_key, user_id }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...defaultParams,
      user_id
    };
    console.log("🗑️ Payload to remove mapped account:", payload);
    const response = await axiosInstance.post("/ajaxfiles/remove_mapped_account", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to remove mapped account");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (remove_mapped_account) failed", err);
    throw err;
  }
};

export const viewLoginMapAccountsAPI = async () => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post("/ajaxfiles/view_login_map_account", defaultParams);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to load mapped accounts");
      return { status: "error", data: [] };
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ API call (view_login_map_account) failed", err);
    throw err;
  }
};

export const placeTrade = async (closetradeData) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = {
      ...closetradeData,
      ...defaultParams,
    };
    console.log("📤 Payload to place trade:", payload);
    const response = await axiosInstance.post("/ajaxfiles/trade_place_v2", payload);

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to place trade");
      return response.data;
    }

    return response.data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ Trade placement failed", err);
    throw err;
  }
};

export const saveEmployee = async (formData, dataStored, isEditMode, editUserId) => {
  try {
    const { name, password, remarks, permissions } = formData;
    const selectedPermissions = Object.keys(permissions).filter(key => permissions[key]);
    const permissionString = selectedPermissions.join(",");
    const defaultParams = await getDefaultParams();

    const basePayload = {
      ...defaultParams,
      name,
      remarks,
      empPermission: permissionString,
    };

    const api = isEditMode
      ? "/ajaxfiles/edit_user_emp"
      : "/ajaxfiles/create_employee";

    const payload = isEditMode
      ? { ...basePayload, change_user_id: editUserId }
      : { ...basePayload, password };

    const { data } = await axiosInstance.post(api, payload);

    if (data?.status === "error") {
      toast.error(data?.message || "Failed to save employee");
      return data;
    }

    return data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ Employee save failed", err);
    throw err;
  }
};

export const getLedgerBalance = async (dataStored, userId) => {
  try {
    if (!dataStored) throw new Error("Session expired. Please log in again.");
    if (!userId) throw new Error("User ID is required");

    const defaultParams = await getDefaultParams();
    const payload = { ...defaultParams, user_id: userId };
    const { data } = await axiosInstance.post("/ajaxfiles/get_ledger_balance", payload);

    if (data?.status === "error") {
      toast.error(data?.message || "Failed to fetch ledger balance");
      return { balance: 0 };
    }

    return data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ Balance fetch failed", err);
    return { balance: 0 };
  }
};

// ===== Centralized Auth/Login and Account Mapping =====
export const loginAPI = async ({ api, values }) => {
  try {
    const { data } = await axiosInstance.post(`/${api}`, values);
    if (data?.status === "error") {
      toast.error(data?.message || "Login failed");
      return data;
    }
    return data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    throw err;
  }
};

export const loginIntoMappedAccountAPI = async ({ user_id }) => {
  try {
    const defaultParams = await getDefaultParams();
    const payload = { ...defaultParams, user_id };
    const { data } = await axiosInstance.post('/ajaxfiles/login_into_mapped_account', payload);

    if (data?.status === "error") {
      toast.error(data?.message || "Failed to login into mapped account");
      return data;
    }

    return data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ Mapped login failed", err);
    throw err;
  }
};

export const addLoginMapAccountAPI = async ({ username, password }) => {
  try {
    const defaultParams = await getDefaultParams();
    const sessionData = JSON.parse(sessionStorage.getItem('data') || '{}');
    const payload = {
      ...defaultParams,
      username,
      password,
      mainLinkId: sessionData.user_id
    };
    const { data } = await axiosInstance.post('/ajaxfiles/add_login_map_account', payload);

    if (data?.status === "error") {
      toast.error(data?.message || "Failed to add login map account");
      return data;
    }

    return data;
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("❌ Add login map failed", err);
    throw err;
  }
};

export const deleteAllLogsAPI = async (password) => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post("/ajaxfiles/setting/admin_change_status_all", {
      ...defaultParams,
      id: "All",
      password,
    });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to delete all logs");
      return { success: false, message: response.data.message };
    }

    return response.data || { success: true };
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error("Error deleting all logs:", err);
    return { success: false, message: err.message };
  }
};

// 🔹 Delete Single Log
export const deleteLogAPI = async (id, password) => {
  try {
    const defaultParams = await getDefaultParams();
    const response = await axiosInstance.post("/ajaxfiles/setting/admin_change_status", {
      ...defaultParams,
      id,
      password,
    });

    if (response?.data?.status === "error") {
      toast.error(response?.data?.message || "Failed to delete log");
      return { success: false, message: response.data.message };
    }

    return response.data || { success: true };
  } catch (err) {
    toast.error(err?.response?.data?.message || "Something went wrong");
    console.error(`Error deleting log ${id}:`, err);
    return { success: false, message: err?.response?.data?.message || err.message };
  }
};

export async function exportTradeApi(payload) {
  try {
    const defaultParams = await getDefaultParams();
    console.log("⬇ Export Payload:", payload);

    const response = await axiosInstance.post(
      "/ajaxfiles/download_csv_trade_book",
      { ...defaultParams, ...payload },
      { responseType: "blob" } // ⬅️ important for CSV/Excel downloads
    );

    return response;
  } catch (error) {
    console.error("❌ Trade export failed:", error);
    throw error;
  }
}
