...
const LedgerPage = () => {
  ...
  useEffect(() => {
    ...
    if (tab === 1 && dataStored?.userType !== "3") fetchSportsLedger();
  }, [tab, dataStored?.user_type]);
  ...
  const filteredLedgerData = sortedLedgerData.filter(item =>
    item.valan_name.toLowerCase().includes(searchQuery.toLowerCase())
  );
    ...
    <Box sx={{ background: theme.palette.background.default, minHeight: '100vh', position: 'relative' }}>
      <Tabs
        ...
        sx={{
          ...
          minHeight: '26px',
          '& .MuiTab-root': { fontSize: '0.7rem', minHeight: '26px', padding: '2px 4px', fontWeight: 600 },
          '& .Mui-selected': {
      ...
      >
        <Tab label="STOCK" />
        <Tab label="SPORT" style={{ display: dataStored?.user_type === "3" ? "none" : "block" }} />
      </Tabs>
        ...
        {loading ? (
          ...
          <>
            ...
            <Box sx={{
              ...
              px: 1.5, py: 1, mb: 1.5, borderRadius: 1.5, boxShadow: 3,
              display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 1, mx: 1
            }}>
              ...
              <Typography fontWeight={700} sx={{ color: openingBalance >= 0 ? profitColor : errorColor, fontSize: '0.9rem' }}>
                {openingBalance.toFixed(2)}
              </Typography>
            </Box>

            {/* Controls: Sort + Filter + Search */}
            <Box sx={{ display: 'flex', justifyContent: 'right', alignItems: 'center', pr: 2, mb: 1, flexWrap: 'wrap', gap: 1 }}>
            ...
            {loading ? (
              ...
              filteredLedgerData.map((item, idx) => {
                ...
                return (
                  <Box
                    key={idx}
                    sx={{
                      px: 1, py: 1, borderRadius: 2, background: bgPaper,
          ...
          <>
            ...
            {sportsData.map((item, idx) => {
              ...
              return (
                ...
                >
                  <Typography fontWeight={600} fontSize="0.85rem">{item.remarks}</Typography>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 0.5 }}>
                    <Typography variant="caption" sx={{ color: theme.palette.text.secondary }}>{item.addded_datetime}</Typography>
                    <Typography
                      variant="caption"
