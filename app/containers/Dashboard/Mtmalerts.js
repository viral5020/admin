...
import { MtmalertsAPI, tradeAutosquareofAPI, tradeEditDeleteLogLogsAPI } from './API/API';
...
const Mtmalerts = ({
    ...
    const fetchLogs = async () => {
        try {
            ...
            const result = await MtmalertsAPI(
                currentPage,
                pageSize,
                searchText,
                market,
                        ...
                        >
                            <CardContent sx={{ p: 1, "&:last-child": { pb: 1 } }}>
                                ...
                                <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                    ...
                                    >
                                        Amt: <strong>{log?.notification_message?.toLocaleString() ?? "0"}</strong>
                                    </Typography>

                                    <Typography
                                        variant="caption"
                                        sx={{ fontSize: "0.7rem", color: theme => theme.palette.text.secondary }}
                                    >
                                        {log?.notification_addedtime ?? "-"}
                                    </Typography>
                                </Box>
                            </CardContent>
                        </Card>
